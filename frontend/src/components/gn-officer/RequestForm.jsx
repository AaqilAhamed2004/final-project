    ...
    <Card className="flex flex-col border-aura-border bg-aura-surface">
      ...
      <div className="flex-1 space-y-5">
        ...
        <div className="flex items-center justify-between p-3 bg-aura-surface border border-aura-border rounded">
          <div>
            <div className="text-sm font-semibold text-aura-text font-sans">Public Relief Board</div>
            <div className="text-[11px] font-mono tracking-wider text-aura-text-muted uppercase mt-0.5">Visible to donors & public</div>
          </div>
          <button
            type="button"
            onClick={() => setIsPublic(p => !p)}
            disabled={isSubmitting}
            className={`w-9 h-5 rounded-full transition-colors relative outline-none focus:ring-2 focus:ring-aura-accent focus:ring-offset-2 ${isPublic ? 'bg-aura-accent' : 'bg-aura-border-strong'}`}
          >
            <span className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${isPublic ? 'translate-x-4' : 'translate-x-0'}`} />
          </button>
        </div>
