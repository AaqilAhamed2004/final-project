...
export default function AnalysisModal({ analysis, isLoading }) {
  if (isLoading) {
    return (
      <div className="p-12 flex flex-col items-center justify-center">
        ...
        <div className="text-xs font-mono text-aura-text-faint tracking-[0.2em] uppercase">Retrieving Tactical Intel...</div>
      </div>
    );
  }
  ...
  if (!analysis) {
    return (
      <div className="p-12 text-center text-aura-text-faint font-mono text-sm uppercase tracking-widest">
  ...
  const getPriorityColor = (level) => {
    switch(level?.toLowerCase()) {
      ...
      default: return 'text-aura-text-faint border-aura-border bg-aura-surface';
    }
  };
  ...
  return (
    <div className="p-8 space-y-8">
      {/* Header Info */}
      <div className="flex justify-between items-start border-b border-aura-border pb-6">
        <div>
          <h2 className="text-2xl font-bold font-sans tracking-tight mb-1">AI Logic Analysis</h2>
          <p className="text-aura-text-faint text-xs font-mono uppercase tracking-widest">AURA Engine Result: {analysis.request_id?.slice(-8)}</p>
        </div>
      ...
      {/* Rationale Section */}
      <div className="bg-aura-surface border border-aura-border rounded-lg p-6">
        <div className="flex items-center gap-2 mb-4 text-aura-amber">
          <Info size={18} />
          <h3 className="text-sm font-mono font-bold tracking-widest uppercase text-aura-text">Analysis Rationale</h3>
        </div>
        <p className="text-sm text-aura-text-muted leading-relaxed font-sans italic">
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="text-sm font-mono font-bold tracking-widest uppercase text-aura-text">Tactical Risk Flags</h3>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {risk_factors && risk_factors.length > 0 ? risk_factors.map((risk, index) => (
            <div key={index} className="flex items-start gap-4 p-4 rounded bg-aura-red/5 border border-aura-red/10">
              ...
              <div className="text-xs font-mono text-aura-text leading-relaxed tracking-tight">{risk}</div>
            </div>
          )) : (
