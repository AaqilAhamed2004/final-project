...
const getCategoryColor = (category) => {
  switch (category?.toLowerCase()) {
    case SUPPLY_CATEGORIES.MEDICINE: return 'bg-aura-red/20 text-[#FF8A8A] border-aura-red/30';
...
export default function InventoryTable({ supplies, onEdit, onDelete }) {
  const columns = [
    {
      header: 'Supply ID',
      cell: (row) => <span className="text-aura-amber font-mono text-sm tracking-wider font-bold" title={row._id || row.id}>{(row._id || row.id)?.slice(-6) || '---'}</span>
    },
    {
      header: 'Name',
      cell: (row) => <span className="font-sans font-bold text-white/90 text-sm tracking-tight">{row.item_name || row.name}</span>
    },
    {
      header: 'Category',
      cell: (row) => (
    ...
    {
      header: 'Quantity',
      cell: (row) => <span className="font-mono text-sm text-white/70">{(row.quantity || 0).toLocaleString()} {row.unit || ''}</span>
    },
    {
      header: 'Actions',
      cell: (row) => (
